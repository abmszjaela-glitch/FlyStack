...
import { Send, X, Minus, Maximize2, RotateCcw, ChevronRight } from "lucide-react";
...
const getReply = async (msg, history) => {
  if (!KODEKLOUD_API_KEY) throw new Error("VITE_KODEKLOUD_API_KEY missing in .env");
  const messages = [
    { role: "system", content: SYSTEM_PROMPT },
    ...history.filter((m) => m.id !== 1).map((m) => ({
  ...
  const res = await fetch(KODEKLOUD_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${KODEKLOUD_API_KEY}` },
  ...
  return data?.choices?.[0]?.message?.content?.trim() || "Sorry, no response received. ✈️";
};
...
export default function Chatbot() {
  ...
  const sendMessage = async (text) => {
    ...
    setInput("");
    setError(null);
    const userMsg = { id: Date.now(), from: "user", text: cleanMsg, time: new Date() };
  ...
  return (
    <div className="font-sans">
      ...
      {/* ── POPUP ── */}
      {open && (
        <div className="fixed bottom-28 right-7 z-[9998] w-[390px] rounded-3xl overflow-hidden flex flex-col bg-white shadow-2xl border border-blue-200 origin-bottom-right animate-[popOpen_0.4s_cubic-bezier(0.34,1.56,0.64,1)_forwards]"
          style={{ boxShadow: "0 2px 4px rgba(23,65,196,0.04), 0 8px 24px rgba(23,65,196,0.1), 0 32px 80px rgba(23,65,196,0.2)" }}>

          {/* Header */}
          <div className="relative overflow-hidden px-5 pt-[18px] pb-5"
            style={{ background: "linear-gradient(135deg, #0e2fa8 0%, #1741c4 40%, #2d63d8 70%, #3b7de8 100%)" }}>
            {/* Animated cloud overlay */}
            <div className="absolute inset-0 animate-[clouds_6s_ease-in-out_infinite_alternate]"
              style={{
                background: "radial-gradient(ellipse 120px 40px at 80% 20%, rgba(255,255,255,0.06) 0%, transparent 70%), radial-gradient(ellipse 80px 30px at 20% 70%, rgba(255,255,255,0.04) 0%, transparent 70%)"
              }} />
            {/* Grid lines */}
            ...
            <div className="relative z-10 flex items-center gap-3.5">
              {/* Plane icon */}
              <div className="w-[52px] h-[52px] rounded-2xl flex items-center justify-center flex-shrink-0 animate-[headPlane_4s_ease-in-out_infinite]"
                style={{
              ...
              <div className="flex-1">
                <h4 className="text-white font-extrabold text-base tracking-tight mb-1" style={{ fontFamily: "Syne, sans-serif" }}>
                  FlyStack Assistant
                </h4>
                <div className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-green-400 animate-[dotPulse_2s_infinite]"
              ...
              <div className="flex gap-1.5 items-center">
                {[
                  { icon: <RotateCcw size={12} />, onClick: clearChat, title: "Clear" },
                  { icon: minimized ? <Maximize2 size={12} /> : <Minus size={12} />, onClick: () => setMinimized(m => !m) },
              ...
              <div className="flex-1 overflow-y-auto px-3.5 py-4 bg-[#f3f7ff] flex flex-col gap-3 max-h-[310px] min-h-[220px] scroll-smooth [&::-webkit-scrollbar]:w-[3px] [&::-webkit-scrollbar-thumb]:bg-blue-200 [&::-webkit-scrollbar-thumb]:rounded-full">
                {messages.map((m) => (
                  <div key={m.id}
                    ...
                    {/* Avatar */}
                    <div className={`w-7 h-7 rounded-[10px] flex-shrink-0 flex items-center justify-center text-[13px] ${
                      m.from === "bot"
                        ? "bg-gradient-to-br from-blue-700 to-blue-500"
                        : "bg-gradient-to-br from-violet-700 to-violet-400"
                    }`}
                      style={{ boxShadow: m.from === "bot" ? "0 3px 10px rgba(23,65,196,0.35)" : "0 3px 10px rgba(109,40,217,0.35)" }}>
                      {m.from === "bot" ? "🤖" : "👤"}
                    </div>

                    <div>
                      {/* Bubble */}
                      <div className={`relative max-w-[74%] px-3.5 py-2.5 text-[13.5px] leading-relaxed ${
                        m.from === "bot"
                        ...
                        style={m.from === "bot"
                          ? { boxShadow: "0 2px 12px rgba(23,65,196,0.08), 0 1px 3px rgba(0,0,0,0.04)" }
                          : { boxShadow: "0 6px 20px rgba(23,65,196,0.4)" }}
                        dangerouslySetInnerHTML={{ __html: parseMarkdown(m.text) }}
                      />
                      {/* Bot bubble tail */}
                      {m.from === "bot" && (
                        <span className="absolute left-[-5px] bottom-2 w-2.5 h-2.5 bg-white border-l border-b border-blue-100 rotate-45"
                          style={{ clipPath: "polygon(0 0, 0 100%, 100% 100%)" }} />
                      )}
                      <div className={`text-[9.5px] text-black/30 mt-1 px-1 ${m.from === "user" ? "text-right" : ""}`}>
                        {fmt(m.time)}
                      </div>
                    </div>
                  </div>
                ...
                {typing && (
                  <div className="flex items-end gap-2">
                    <div className="w-7 h-7 rounded-[10px] flex-shrink-0 flex items-center justify-center text-[13px] bg-gradient-to-br from-blue-700 to-blue-500"
                      style={{ boxShadow: "0 3px 10px rgba(23,65,196,0.35)" }}>🤖</div>
                    <div className="bg-white rounded-[18px] rounded-bl-[4px] px-4 py-3 flex gap-1.5 items-center border border-blue-100"
                      style={{ boxShadow: "0 2px 12px rgba(23,65,196,0.08)" }}>
                      {[0, 0.2, 0.4].map((delay, i) => (
                        <span key={i} className="w-[7px] h-[7px] rounded-full bg-gradient-to-br from-blue-700 to-blue-500 animate-[typingDot_1.3s_ease_infinite]"
              ...
              <div className="px-3.5 pt-3 pb-3.5 bg-white border-t border-blue-50">
                <div className="flex items-center gap-2 bg-[#f0f5ff] rounded-[14px] px-3.5 py-2 border-[1.5px] border-blue-200 transition-all duration-200 focus-within:border-blue-500 focus-within:shadow-[0_0_0_4px_rgba(59,125,232,0.1)] focus-within:bg-white">
                  ...
                  <button
                    ...
                    {typing
                      ? <svg className="animate-spin" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round"><path d="M21 12a9 9 0 1 1-6.219-8.56" /></svg>
                      : <Send size={15} />}
                  </button>
                </div>
                <p className="text-center text-[10px] text-black/25 mt-2 tracking-wide">
                  Powered by <span className="text-blue-500 font-semibold">FlyStack AI</span> · Your intelligent aviation assistant
                </p>
              </div>
            </>
          )}
      ...
      <button
        ...
        style={{ background: "linear-gradient(145deg, #1741c4, #3b7de8, #5a9cf5)", boxShadow: "0 10px 32px rgba(23,65,196,0.5)" }}>
        {open ? (
    ...
    <svg width={size} height={size} viewBox="0 0 40 34" fill="none" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="pg1" x1="0" y1="0" x2="40" y2="34" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#e2eaf8" /><stop offset="45%" stopColor="#f8faff" /><stop offset="100%" stopColor="#b0c4e8" />
        </linearGradient>
        ...
        <linearGradient id="pg5" x1="6" y1="17" x2="18" y2="22" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#475569" /><stop offset="100%" stopColor="#94a3b8" />
        </linearGradient>
        ...
        <filter id="pShadow" x="-20%" y="-20%" width="140%" height="140%">
          <feDropShadow dx="0" dy="3" stdDeviation="2.5" floodColor="#1741c4" floodOpacity="0.3" />
        </filter>
      </defs>
